			...
			<Tab.List className='flex space-x-2 items-stretch mb-10 bg-white shadow-2xl shadow-blue-900/5 p-1.5 rounded-3xl'>
				{items.map((item, index) => (
					<Tab key={index} className='flex-1'>
						{({ selected }) => (
							<div
								className={`py-3 px-4 h-full flex items-center justify-center hover:bg-slate-50 rounded-2xl transition after:content-[""] after:absolute relative after:h-[0.1875rem] after:rounded-full after:transition-all after:left-1/2 after:-translate-x-1/2 after:-bottom-1.5 font-medium ${
									selected
